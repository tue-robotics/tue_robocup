...
if __name__ == "__main__":
    ...
    entity_id = "test_item"
    pose = FrameStamped(
        frame=kdl.Frame(kdl.Rotation.RPY(0.0, 0.0, 0.0), kdl.Vector(0.0, 0.0, 0.0)),
        stamp=rospy.Time.now(),
        frame_id="map",
    )
    person_properties = PersonProperties(
        "John", 20, "happy", 0.0, 0.9, "left", "standing", 0.9, ["orange", "black"], "LStanding", "", 0, "map"
    )
    ...
    item = Entity(
        entity_id, "person", pose.header.frame_id, pose.frame, shape, None, None, rospy.Time.now(), person_properties
    )
    ...
    person_properties2 = PersonProperties(
        "Anna", 20, "happy", 1.0, 0.9, "left", "standing", 0.9, ["orange", "black"], ["LStanding"], "", 0, "map"
    )
    item2 = Entity(
        entity_id2, "person", pose.header.frame_id, pose.frame, shape, None, None, rospy.Time.now(), person_properties2
    )
