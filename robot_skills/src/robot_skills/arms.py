...
class SuctionGripperArm(Arm):
    def __init__(self, robot_name, tf_listener, get_joint_states, side):
        ...
        :param side: left or right
        """
        super(SuctionGripperArm, self).__init__(robot_name, tf_listener, get_joint_states, side)
        # Get stuff from the parameter server
        offset_suction = self.load_param('skills/arm/' + self.side + '_suction/grasp_offset/')
        self.offset_suction = kdl.Frame(kdl.Rotation.RPY(offset_suction["roll"], offset_suction["pitch"], offset_suction["yaw"]),
                                        kdl.Vector(offset_suction["x"], offset_suction["y"], offset_suction["z"]))

        # Init gripper actionlib
        self._ac_suction = self.create_simple_action_client(
            "/" + robot_name + "/suction_control", SuctionControlAction)
    ...
    def send_gripper_goal_suction(self, sucking, timeout=5.0):
        ...
        if isinstance(sucking, bool):
            goal.suction_on.data = sucking
        else:
            rospy.logerr('State shoulde be true or false, now it is {0}'.format(sucking))
            return False

        self._ac_suction.send_goal(goal)

        if not sucking:
            if self.occupied_by_suction is not None:
                rospy.logerr("send_gripper_goal open is called but there is still an entity with id '%s' \
                occupying the gripper, please update the world model and remove this entity" % self.occupied_by_suction.id)
            self.occupied_by_suction = None
