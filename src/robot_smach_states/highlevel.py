...
class Navigate_to_queryoutcome_waypoint_location(navigation.Navigate_abstract):
    ...
    def __init__(self, robot, query, X="X", Y="Y", Phi="Phi"):
        navigation.Navigate_abstract.__init__(self, robot)
...
class Navigate_to_queryoutcome_waypoint_location2(navigation.Navigate_abstract):  
    """Move to the output of a query, which is passed to this state as a Term from the reasoner-module.
...
class Navigate_to_queryoutcome_waypoint_location3(navigation.Navigate_abstract):  
    """Move to the output of a query, which is passed to this state as a Term from the reasoner-module.
...
class GotoMeetingPointEGPSR(smach.StateMachine):
    """Initialize, wait for the door to be opened and drive inside"""
    def __init__(self, robot):
        ...
        with self:
            ...
            smach.StateMachine.add('GO_TO_MEETING_POINT_EGPSR_1', 
                                    Navigate_to_queryoutcome_waypoint_location(robot, navigate_meeting_point_1, X="X", Y="Y", Phi="Phi"),
            ...
            smach.StateMachine.add("FAILED_FIRST_ATTEMPT",
                                    human_interaction.Say(robot, ["I was not able to reach the meeting point at first attempt, I will try it again."]),
            ...
            smach.StateMachine.add('GO_TO_MEETING_POINT_EGPSR_2', 
                                    Navigate_to_queryoutcome_waypoint_location(robot, navigate_meeting_point_2, X="X", Y="Y", Phi="Phi"),
                                    transitions={   'arrived':'succeeded', 
            ...
            smach.StateMachine.add("FAILED_SECOND_ATTEMPT",
                                    human_interaction.Say(robot, [  "Also my second attempt was not succesful. One last try."]),
            ...
            smach.StateMachine.add('GO_TO_MEETING_POINT_EGPSR_3', 
                                    Navigate_to_queryoutcome_waypoint_location(robot, navigate_meeting_point_3, X="X", Y="Y", Phi="Phi"),
                                    transitions={   'arrived':'succeeded', 
