...
class LearnPersonFace(smach.State):
    ...
    def execute(self, userdata=None):
        ...
        self.robot.speech.speak("Now " + userdata.personName_in + ", let me lear your face.", block=False)
...
# TODO: SHOULD THIS QUERY BELOW HAVE A ompound("pose_2d", "X", "Y", "Phi") ???
...
class NavToLastKnowLoc(smach.State):
    ...
    def execute(self, userdata=None):
        ...
        # # goals for the last know locations of people
        # goals = self.robot.reasoner.query(qGoals)

        # if goals:
        #     goal_answer = goals[0]
        #     self.robot.speech.speak("I'm going to {0}'s last know location.".format(goal_answer["Person"]), block=False)
        ...
        # else:
        #     # self.robot.speech.speak("I don't where else to search for people", block=False)
        #     return "visited_all"

        # else:
        #     # get the waypoint of where to search, party_room_lookout
        #     qGoals = self.robot.reasoner.query(Conjunction(  Compound("=", "Waypoint", Compound("party_room_lookout", "W")),
        #                                              Compound("waypoint", "Waypoint", Compound("pose_2d", "X", "Y", "Phi")),
        #                                              Compound("not", Compound("visited", "Waypoint"))))

        #     # goals for the lookout points
        #     goalsB = self.robot.reasoner.query(qGoals)

        #     if goalsB:
        #         goal_answer = goalsB[0]
        #         self.robot.speech.speak("I'm going to another lookout point.", block=False)
        #     else:
        #         self.robot.speech.speak("I don't where else to search for people", block=False)
        #         return "visited_all"
...
class PendingOrders(smach.State):
    ...
    def execute(self, userdata=None):
        ...
        if nOrders < SIMULTANEOUS_ORDERS:
...
class PreparePickup(smach.State):
    ...
    def execute(self, userdata = None):
        ...
        if not "left_arm" in carryingNames:
            carryingLoc = "left_arm"
        elif not "right_arm" in carryingNames:
            carryingLoc = "right_arm"
        elif not "basket" in carryingNames:
            carryingLoc = "basket"
...
class ResetSearchedLocations(smach.State):
    ...
    def execute(self, userdata = None):
        ...
        if self.resetCount == 0:
            self.robot.speech.speak(robot, ["I could not pick up the drink you wanted. I'll try again.", 
                                            "I failed to grab the object you wanted. Let's try again."], block=False),
            self.resetCount+=1;
...
class PersonFound(smach.State):
    ...
    def execute(self, userdata):
        ...
        self.robot.head.reset_position()
        self.robot.speech.speak("Hello, could you look at my face so I can recognize you.", block=False)
        ...
        # TODO: CHANGE THIS INTO A WAITING MACHINE LIKE WAIT FOR PERSON!
            ...
            if not personName:
                ...
                for name_possibility in personNames:
                    ...
                    if personName == name_possibility:
                        ...
                        if str(carryingLoc) == "left_arm":
                            return 'handover_left'
                        elif str(carryingLoc) == "righ_arm":
                            return 'handover_right'
                        elif str(carryingLoc) == "basket":
                            ...
                            rospy.logwarn("\t\t[Cocktail Party] The CarryingLoc is not valid (carryingLoc=[{1}]). Classifying the person as unknown\n".format(
                                str(carryingLoc))) 
                            return 'unknown'
    ...
    def __init__(self, robot):
        ...
        with self:
            ...
            smach.StateMachine.add( 'PLEASE_TAKE',
                                    Say(robot, ["Please hold it, I'm going to let go.", "Please grab the drink, I'm releasing"]),
...
class RetractServedPerson(smach.State):
    ...
    def execute(self, userdata):
        ...
        # personName = str(userdata.person_in)
        # drinkName = str(userdata.drink_in)
        
        # TODO PASS THE VARIABLES THROUGH USERDATA AND NOT QUERY
        deliveryRes = self.robot.reasoner.query(Compound("delivering",
                                                Compound("delivery", "Person", "Drink")))
    
        if not deliveryRes:
            rospy.loginfo("\t\t[Cocktail Party] No information on this delivery.\n")
            return 'failed'
        else:
        ...
        if personName and drinkName:
            ...
            res1 = self.robot.reasoner.query(Compound("people_served_count", "X"))
            res2 = self.robot.reasoner.query(Compound("goal", Compound("serve", "ObjectID", "Person", "Drink", "LastKnowPose")))

            nServed = float(res1[0]["X"]) + 1

            rospy.loginfo("\t\t[Cocktail Party]Hurray! Another drink was served (total served: {0} + {1})\n".format(float(res1[0]["X"]), 1))
            ...
            if len(carryingRes) == 0:
                ...
                rospy.loginfo("\t\t[Cocktail Party] Still have to deliver {0} drink(s)\n".format(len(carryingRes)))
                return 'done'

        else:
            rospy.loginfo("\t\t[Cocktail Party] Unable to retract the person and drink served\n")
            return 'failed'
...
class DropInBasket(smach.StateMachine):
    def __init__(self, robot):
        ...
        with self:
            ...
            smach.StateMachine.add( 'SAY_STORING_DRINK',
                                    Say(robot, ["I'm sotring this drink in my basket"], block=False),
                                    transitions={"spoken":"done"})

            #TODO: ADD STATES THAT DROP THE DRINK IN THE BASKET
...
#----------------------------------------WAIT FOR PEOPLE TO COME IN FRONT (WORST CASE SCENARIO)------------------
...
#--------------------------------------------FIND THE REQUESTED DRINKS---------------------------------------------
            ...
            with findDrinksContainer:
                ...
                smach.StateMachine.add( 'SAY_DRINK_PICKED_UP',
                                        Say(robot, "I picked up one of the drinks requested", block=False),
                ...
                #                                     'I failed to grab the object you wanted.'], block=False),
                #                         transitions={   'spoken':'LOOK_FOR_DRINK' })
            ...
            with deliverDrinksContainer:      
                ...
                smach.StateMachine.add( 'PERSON_FOUND',
                                        ...
                                        transitions={   'handover_left':'HANDOVER_DRINK_LEFT',
                                                        ...
                                                        'no_people':'NAV_TO_LAST_KNOWN_LOCATION'})
