...
query_dropoff_loc = Conjunction(
                        ...
                        Compound("storage_class",       "ObjectType",       "Disposal_type"), #Find AT what sort of thing it should be disposed, e.g. a trashbi
...
query_dropoff_loc_backup = Conjunction( Compound("instance_of", "Dispose_to_object",  "trashbin"), #Find objects of that are of type trashbin
                                            Compound("point_of_interest",  "Dispose_to_object",  Compound("point_3d", "X", "Y", "Z"))) #Get locations of those 
...
if __name__ == "__main__":
    rospy.init_node('knowledge_debugger')
        ...
        try:
            for name, term in queries.iteritems():
                ...
                if new_mask[name] != old_mask[name]:
                    rospy.loginfo("query {0} changed. Was {1}, now is {2}".format(name, pprint.pformat(old_mask[name]), pprint.pformat(new_mask[name])))
            old_mask = new_mask
