...
class ActiveGraspDetector(smach.State):
    ...
    def __init__(self, arm_designator, robot):
        """
        State for detecting whether or not the robot is holding something
    ...
    def execute(self, userdata=None):
        ...
        if self.first_position is None:
            ...
            if self.second_position is not None and abs(self.first_position - self.second_position) < self.threshold_difference:
                return 'succeeded'
            else:
                return 'failed'
