...
bool CarrotPlanner::isClearLine(tf::Vector3 &goal){
    ...
    ROS_WARN("Intended driving direction outside field of view LRF, only considering orientation to avoid collisions");
...
void CarrotPlanner::determineDesiredVelocity(tf::Vector3 e_pos, double e_theta, const geometry_msgs::Twist &current_vel,
    ...
    if (e_norm > 0) {
        ...
        ROS_WARN("e_norm = %f, e_theta = %f", e_norm, e_theta);
    }
