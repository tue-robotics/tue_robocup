    ...
    def execute(self, userdata):
        ...
        if answers:
            ...
            if self.robot.base.send_goal(desired_base_poses_MAP[0], time=60):
                ...
                #self.robot.base.wait(timeout=60)
...
class UpdateObjectPose(smach.State):
    ...
    def execute(self, gl):
        ...
        if answers:
            answer = answers[0] #TODO Loy/Sjoerd: sort answers by distance to gripper/base? 
            target_point = msgs.PointStamped(float(answer["X"]), float(answer["Y"]), float(answer["Z"]), frame_id = "/amigo/base_link")
...
class Grab(smach.State):
    ...
    def execute(self, gl):
        ...
        if answers:
            ...
            target_position.header.frame_id = "/amigo/base_link"
        ...
        # No transformation required, answer is in base_link for r3-Cop demoo
        target_position_bl = target_position; 
        ...
        target_position_grippoint = transformations.tf_transform(target_position, "/amigo/base_link", self.end_effector_frame_id, tf_listener=self.robot.tf_listener)
...
class ArmToQueryPoint(smach.State):
    ...
    def execute(self, userdata):
        ...
        if not answers:
            rospy.loginfo("No answers for query {0}".format(self.query))
            return 'failed'
        answer = answers[0] #TODO Loy/Sjoerd: sort answers by distance to gripper/base? 
