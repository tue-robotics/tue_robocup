...
bool moveHead(double pan, double tilt, bool block = true)
{
    ...
    if (block) {
		...
		ac_head_ref_->waitForResult(ros::Duration(5.0));
		//ac_head_ref_->sendGoalAndWait(head_ref, ros::Duration(5.0));
		
		if (ac_head_ref_->getState() != actionlib::SimpleClientGoalState::SUCCEEDED)
		{
			if (ac_head_ref_->getState() == actionlib::SimpleClientGoalState::PENDING) {
				ROS_WARN("Head could not reach target position -> PENDING" );
			}
			if (ac_head_ref_->getState() == actionlib::SimpleClientGoalState::ACTIVE) {
				ROS_WARN("Head could not reach target position -> ACTIVE" );
			}
			if (ac_head_ref_->getState() == actionlib::SimpleClientGoalState::RECALLED) {
				ROS_WARN("Head could not reach target position -> RECALLED" );
			}
			if (ac_head_ref_->getState() == actionlib::SimpleClientGoalState::REJECTED) {
				ROS_WARN("Head could not reach target position -> REJECTED" );
			}
			if (ac_head_ref_->getState() == actionlib::SimpleClientGoalState::PREEMPTED) {
				ROS_WARN("Head could not reach target position -> PREEMPTED" );
			}
			if (ac_head_ref_->getState() == actionlib::SimpleClientGoalState::ABORTED) {
				ROS_WARN("Head could not reach target position -> ABORTED" );
			}
			if (ac_head_ref_->getState() == actionlib::SimpleClientGoalState::LOST) {
				ROS_WARN("Head could not reach target position -> LOST" );
			}
...
int main(int argc, char **argv) {
    ...
    while (!operator_found)
    {
		...
		if (headposition == 3) {
			moveHead(-0.5, 0.0, true);
		}
		if (headposition == 4) {
			moveHead(0.0, 0.0, true);
		}
		if (headposition == 5) {
			moveHead(0.5, 0.0, true);
		}
