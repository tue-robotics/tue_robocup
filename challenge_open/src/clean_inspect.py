        ...
        with self:
            ...
            for i, segment_area in enumerate(segment_areas):
                ...
                smach.StateMachine.add("NAVIGATE_%d" % i,
                                       robot_smach_states.NavigateToSymbolic(robot, {e_des: navigate_area,
                                                                                     room_des: "in"},
                                                                             e_des),
                ...
                smach.StateMachine.add("SAY_UNREACHABLE_%d" % i,
                                       robot_smach_states.Say(robot, ["I failed to inspect the %s" % location_id],
                                                              block=True),
                                       transitions={"spoken": next_state})

                smach.StateMachine.add("HANDLE_DETECTED_ENTITIES_%d" % i,
                                       HandleDetectedEntities(robot, e_classifications_des, location_id,
                                                              segment_area),
