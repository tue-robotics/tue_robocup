    ...
    def __init__(self, robot, room_id):
        ...
        def _data_association_persons_and_show_image_on_screen(_):
            ...
            for i, person_detection in enumerate(person_detection_clusters):
                ...
                try:
                    px_image = min(max(0, px - calculated_width / 2), floorplan_width - calculated_width / 2 - 1)
                    py_image = min(max(0, py - desired_height / 2), floorplan_height - desired_height / 2 - 1)
